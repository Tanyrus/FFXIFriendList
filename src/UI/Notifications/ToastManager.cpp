...
// Helper function to get viewport size from ImGui (outside namespace to access ImGui)
#ifndef BUILDING_TESTS
static ImVec2 getViewportSizeHelper(IGuiManager* guiManager) {
    if (!guiManager) {
        return ImVec2(1920.0f, 1080.0f);  // Default fallback
    }
    
    // Workaround: Create a temporary invisible window to query viewport size
    // This is a common pattern when you need viewport info outside of a window context
    // We'll create a window with flags that make it invisible and non-interactive,
    // then query its viewport size
    
    const char* tempWindowId = "##ToastViewportQuery";
    
    ImGuiWindowFlags tempFlags = 
    ...
    // Position the window at (0, 0) and make it full-screen to get accurate viewport size
    // We use a large size to ensure it covers the entire viewport
    guiManager->SetNextWindowPos(ImVec2(0.0f, 0.0f), 0);  // ImGuiCond_Always
    guiManager->SetNextWindowSize(ImVec2(10000.0f, 10000.0f), 0);  // Large size to cover viewport
    
    // Begin the temporary window
    bool windowOpened = guiManager->Begin(tempWindowId, nullptr, tempFlags);
    
    if (windowOpened) {
        // In ImGui, each window has a viewport, and we can query it
        // Use GetContentRegionAvail() which is available on IGuiManager
        // GetContentRegionAvail() returns the available content region
        // For a full-screen window, this approximates the viewport size
        
        ImVec2 contentAvail = guiManager->GetContentRegionAvail();
        
        // GetContentRegionAvail() gives us the available content region
        // For a full-screen window, this should be close to the viewport size
        // However, we need to account for window padding and decoration
        // We can approximate by adding padding (typically 8-16px per side)
        
        ImVec2 viewportSize;
        if (contentAvail.x > 0.0f && contentAvail.y > 0.0f) {
            // Add estimated padding (16px per side = 32px total)
            // This is an approximation - the actual viewport might be slightly different
            viewportSize = ImVec2(contentAvail.x + 32.0f, contentAvail.y + 32.0f);
        } else {
            // Fallback to default if contentAvail is invalid
            viewportSize = ImVec2(1920.0f, 1080.0f);
        }
        
        // End the temporary window
        guiManager->End();
...
// Helper function to calculate default position (TopLeft) - kept for compatibility but not used
static ImVec2 calculateDefaultPositionHelper(const ImVec2& viewportSize) {
    // Default: TopLeft corner
    const float TOAST_PADDING = 15.0f;
    return ImVec2(TOAST_PADDING, TOAST_PADDING);
}
