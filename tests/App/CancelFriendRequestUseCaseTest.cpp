...
TEST_CASE("CancelFriendRequestUseCase - Success", "[CancelFriendRequestUseCase]") {
    ...
    response.body = R"({"protocolVersion":"2.0.0","type":"CancelFriendRequestResponse","success":true})";
    
    netClient->setResponseCallback([response](const std::string& url, const std::string&, const std::string&) {
        if (url.find("/api/friends/requests/cancel") != std::string::npos) {
            return response;
        }
        return HttpResponse{404, "", "URL not found"};
    });
    
    CancelFriendRequestUseCase useCase(*netClient, *clock, *logger);
    
    auto result = useCase.cancelRequest("test-api-key", "testchar", "request123");
    
    REQUIRE(result.success == true);
    REQUIRE(result.errorCode.empty());
    REQUIRE(result.userMessage == "Request canceled.");
}

TEST_CASE("CancelFriendRequestUseCase - Request ID validation", "[CancelFriendRequestUseCase]") {
    auto netClient = std::make_unique<FakeNetClient>();
    auto clock = std::make_unique<FakeClock>();
    auto logger = std::make_unique<FakeLogger>();
    ...
    SECTION("Empty request ID") {
        auto result = useCase.cancelRequest("test-api-key", "testchar", "");
        REQUIRE(result.success == false);
        REQUIRE(result.userMessage.find("required") != std::string::npos);
    }
    
    SECTION("Empty API key") {
        auto result = useCase.cancelRequest("", "testchar", "request123");
        REQUIRE(result.success == false);
        REQUIRE(result.userMessage.find("required") != std::string::npos);
    }
    
    SECTION("Empty character name") {
        auto result = useCase.cancelRequest("test-api-key", "", "request123");
        REQUIRE(result.success == false);
        REQUIRE(result.userMessage.find("required") != std::string::npos);
    }
}

TEST_CASE("CancelFriendRequestUseCase - HTTP error", "[CancelFriendRequestUseCase]") {
    auto netClient = std::make_unique<FakeNetClient>();
    auto clock = std::make_unique<FakeClock>();
    auto logger = std::make_unique<FakeLogger>();
...
TEST_CASE("CancelFriendRequestUseCase - Invalid response format", "[CancelFriendRequestUseCase]") {
    auto netClient = std::make_unique<FakeNetClient>();
    auto clock = std::make_unique<FakeClock>();
    auto logger = std::make_unique<FakeLogger>();
...
TEST_CASE("CancelFriendRequestUseCase - Retry logic", "[CancelFriendRequestUseCase]") {
    auto netClient = std::make_unique<FakeNetClient>();
    auto clock = std::make_unique<FakeClock>();
    auto logger = std::make_unique<FakeLogger>();
